...
try:
    with open(os.path.join(stubdir, pim2)) as f:
...
if stepsuccess[1]:
    try:
        ...
        if num_vertx <= 0:
            stepsuccess[2] = False
            print('There was a problem reading {} in order to determine number of vertices in 3D meshes!'.format(
                meshtxtfile))
    except:
        stepsuccess[2] = False
...
if all(stepsuccess):
    ...
    print('Expected number of vertices per mesh = {0}, of which {1} will be extracted'.format(num_vertx,
                                                                                              mpoints.shape[0]))
            ...
            for counter, ID in enumerate(IDlist):
                if os.path.exists(os.path.join(stubdir, ID)):
                    if os.path.exists(os.path.join(stubdir, ID, 'motion')):
                        frames_file_list = [os.path.join(stubdir, ID, 'motion/RV_fr' + '{:0>2}'.format(b) + '.txt') for
                                            b in range(numframes)]
                        if all_files_exist(frames_file_list):
                            ...
                            if nframes == numframes:
                                if np.sum([file_len(frames_file_list[i]) == num_vertx for i in
                                ...
                                #               else: print(ID + ' RV files do not have ' + str(num_vertx) + ' vertices')
                                    ...
                                    print(
                                        '{0} : wrong # of vertices, expected {1} for all {2} frames but got {3}'.format(
                                            ID, num_vertx, numframes,
                                            str([file_len(frames_file_list[i]) for i in range(numframes)])))
                            else:
                                print('{0} : RV files exist but not {1} in number. Skipping to next ID....'.format(ID,
                                                                                                                   numframes))
                        else:
...
if any(validIDs):
    ...
    print('No valid mesh motion data could be read!')
...
if any(validIDs):
    ...
    if os.path.exists(outcomefile):
        try:
            ...
            oreadable = False
        if oreadable:
            print('Outcome file {0} read: {1} rows and {2} columns...'.format(outcomefile, outcome_df.shape[0],
                                                                              outcome_df.shape[1]))
            if len(outcome_df.columns) != 3:
                ...
                try:
                    ocorrfmt = np.all([i and j for (i, j) in zip([l in [0, 1] for l in list(outcome_df.status)],
                                                                 [k >= 0 for k in list(outcome_df.time)])])
                except:
                    ofmtcorr1 = False
                    ocorrfmt = False
                if not (ofmtcorr1 and ocorrfmt):
                    ...
                    if ofmtcorr1 == True and ocorrfmt == False:
                        aw = np.argwhere([not (i and j) for (i, j) in
                                          zip([l in [0, 1] for l in list(outcome_df.status)],
                                              [k >= 0 for k in list(outcome_df.time)])])
                        if aw.shape[0] > 0:
                            print(
                                '{} {rw} {w} problematic: '.format(aw.shape[0], rw='rows' if aw.shape[0] > 1 else 'row',
                                                                   w='were' if aw.shape[0] > 1 else 'was'))
                    ...
                    if any(validIDs):
                        ...
                        if len(IDlist_woutc) == 0:
                            print('None of the IDs from the mesh motion data were found in outcome file {}'.format(
                                outcomefile))
                        else:
                            print('{1} of {2} valid IDs from mesh motion data were found in outcome file {0}'.format(
                                outcomefile, len(IDlist_woutc), len(IDlist_valids)))
                            if len(IDlist_woutc) < len(IDlist_valids):
                                print(
                                    'The following IDs from the mesh motion data were not found in outcome file {} :'.format(
                                        outcomefile))
                            ...
                            assert xymatch, 'ERROR: mesh motion (x) data has {1} rows while outcome (y) data has {0} rows'.format(
                                y.shape[0], xshp[0])
                            if xymatch:
                                ...
                                print(
                                    'Mesh motion and corresponding survival data for {0} subjects has been saved in {1}'.format(
                                        xshp[0], pklpath))
        else:
            pass
    else:
