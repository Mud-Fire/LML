...
# Define Cox PH partial likelihood function loss. Arguments: E (censoring status), risk (risk [log hazard ratio]
# predicted by network) for batch of input subjects As defined, this function requires that all subjects in input
# batch must be sorted in descending order of survival/censoring time (i.e. arguments E and risk will be in this order)
...
def DL_single_run(xtr, ytr, units1, units2, dro, lr, l1r, alpha, batchsize, numepochs):
    # Data preparation: create X, E and TM where X=input vector, E=censoring status and T=survival time. Apply
    # formatting (X and T as 'float32', E as 'int32')
    X_tr, E_tr, TM_tr = prepare_data(xtr, ytr[:, 0, np.newaxis], ytr[:, 1])

    # Arrange data into minibatches (based on specified batch size), and within each minibatch, sort in descending
    # order of survival/censoring time (see explanation of Cox PH loss function definition)
    X_tr, E_tr, TM_tr, _ = sort4minibatches(X_tr, E_tr, TM_tr, batchsize)

    # before defining network architecture, clear current computation graph (if one exists), and specify input
    # dimensionality
    K.clear_session()
    ...
    # Model compilation
    optimdef = Adam(lr=lr)
    model.compile(loss=[keras.losses.mean_squared_error, _negative_log_likelihood], loss_weights=[alpha, 1 - alpha],
                  optimizer=optimdef, metrics={'decoded': keras.metrics.mean_squared_error})
